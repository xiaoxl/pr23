...
::: {#exr-}
Write a function to reverse a `ndarray` (first element becomes last).
:::
...
Consider two `ndarrays` of the same length `x` and `y`. Compare them entry by entry. We would like to know how many are the same.
