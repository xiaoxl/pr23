...
The basic idea is to use a `list` of indexes to select entries. 
...
If only one `list` is given, this `list` is considered as the list of row indexes. The resulted `ndarray` is always 1d.
