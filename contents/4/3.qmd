...
These `DataFrame` are provided by `tidyr`. We will talk about them again when we get to R.
...
4. Spread across two `DataFrame`s: `table4a` and `table4b`:
...
If we prepare two columns from the beginning, we could directly get two columns.
